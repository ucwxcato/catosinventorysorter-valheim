...
The sorter keeps the hotbar row, equipped items, and bound items in place. It changes only `ItemData.m_gridPos`, then calls Valheim's `Inventory.Changed` callback so the game's normal inventory-change listeners update. It does not open containers, move items between inventories, or add a server plugin or custom network messages.
